...
from utils_adastar import get_dataloader_adastar, update_winlose_new, update_minheap_new, log_stable_minheap_new
...
def eval_examples(args, model, rank, dataset_train, tokenizer, gen_length, n_shot_prompts, n_shot_prompts_hint, winlose):
    ...
    for i in range(len(dataset_train)):
        if world_size * i + rank >= len(dataset_train): break
        gpu_data.append(dataset_train[world_size*i+rank])
    ...
    def fill_buffer():
        while len(buffer) < args.batch_size and gpu_data:
            buffer.append(gpu_data.popleft())
            hint.append(False)
            id_idx = buffer[-1]["idx"]
...
def eval_examples_nohint(args, model, rank, dataset_train, tokenizer, gen_length, n_shot_prompts, n_shot_prompts_hint, winlose):
    generate_fn = model.module.generate if hasattr(model, 'module') else model.generate
    world_size = dist.get_world_size()

    correct_total = torch.zeros(2).to(rank)
    gpu_correct_total = torch.zeros(2).to(rank)
...
def evaluate(args, model, rank, world_size, test_loader, tokenizer, gen_length, target_save, n_shot_prompts, n_shot_prompts_hint, winlose):
    model.eval()

    if args.split == "train":
        if args.no_hint:
            gpu_rationale_dataset, gpu_data_idx, gpu_winlose, all_correct_total = eval_examples_nohint(args, model, rank, test_loader, tokenizer, gen_length, n_shot_prompts, n_shot_prompts_hint, winlose)
        else:
            gpu_rationale_dataset, gpu_data_idx, gpu_winlose, all_correct_total = eval_examples(args, model, rank, test_loader, tokenizer, gen_length, n_shot_prompts, n_shot_prompts_hint, winlose)
    else:
...
def fsdp_main(rank, world_size, args):
    ...
    dataset_train, winlose, heap = get_dataloader_adastar(args, tokenizer, rank, world_size)
    ...
    if args.split == "train":
        ...
        all_rationale_dataset, all_data_idx, all_winlose, all_correct_total = evaluate(args, model, rank, world_size, dataset_train, tokenizer, args.gen_length, args.target_save, n_shot_prompts, n_shot_prompts_hint, winlose)
    ...
    if rank == 0:
        ...
        updated_winlose = update_winlose_new(winlose, all_winlose, num_used_data)
        updated_minheap = update_minheap_new(updated_winlose, heap, num_used_data)
